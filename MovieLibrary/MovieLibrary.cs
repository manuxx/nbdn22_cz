...
namespace TrainingPrep.collections
{
    public class MovieLibrary
    {
        ...
        {
            return movies.ThatSatisfy(Movie.IsPublishedBetween(start_year, end_year));
        }

        public IEnumerable<Movie> all_movies_published_by_pixar_or_disney()
        {
            return movies.ThatSatisfy(Movie.IsProducedBy(ProductionStudio.Disney, ProductionStudio.Pixar));
        }

        public IEnumerable<Movie> all_movies_not_published_by_pixar()
        {
            return movies.ThatSatisfy(new NegationCriteria<Movie>(Movie.IsProducedBy(ProductionStudio.Pixar)));
        }

        public IEnumerable<Movie> all_kid_movies_published_after(int year)
        {
            return movies.ThatSatisfy(new ConjunctionCriteria(
                Movie.IsPublishedAfter(year), Movie.IsOfGenre(Genre.kids)));
        }

        public IEnumerable<Movie> all_horror_or_action()
        {
            return movies.ThatSatisfy(Movie.IsOfGenre(Genre.horror, Genre.action));
        }

        public IEnumerable<Movie> all_MGM_or_comedy()
        {
            return movies.ThatSatisfy(new AlternativeCriteria(
                Movie.IsOfGenre(Genre.comedy), Movie.IsProducedBy(ProductionStudio.MGM)));
        }
    }
}